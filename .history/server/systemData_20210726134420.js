...
const sysDataRefresh = (sysData) => {
    ...
    for (let j = 0; j<temp.length; j++) {
        ...
        if (partitionName != undefined) {
            ...
            let partitionUsed = diskData[4];
            let partitionTotal = diskData[3];
            let partitionUsedPercent = ((partitionUsed/partitionTotal)*100).toFixed(1);
            diskData.push(partitionUsedPercent);
            }
    }
