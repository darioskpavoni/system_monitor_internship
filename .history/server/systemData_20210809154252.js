...
// LINUX
  ...
  const output = execSync("sudo df -h | grep ^/dev", { encoding: "utf-8" });
  let parsedOutput = output.split(/\r?\n/);
  ...
  for (let j = 0; j < temp.length; j++) {
    temp[j] = temp[j].split(" ");
    let partitionName = temp[j][1];
    if (partitionName != undefined) {
      if (!temp[j][1].includes("M")) {
        diskData.push(temp[j]);
      }
    }
  }
