...
// LINUX
  ...
  for (let i = 0; i < temp2.length; i++) {
    ...
    let partitionFreePercent = 100 - partitionUsedPercent;

    if (partitionSize.includes("G")) {
      diskData[i] = [];
      diskData[i].push(partitionName);
      diskData[i].push(partitionSizeGB);
      diskData[i].push(partitionUsedGB);
      diskData[i].push(partitionFreeGB);
      diskData[i].push(partitionUsedPercent);
