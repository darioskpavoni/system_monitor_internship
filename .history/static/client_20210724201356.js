...
socket.on('sysData', (sysData) => {
    if(!document.getElementById(sysData.id)) {
        ...
        <td class='DISKused-container'>Initializing...</td>`;

        let diskData = '';
        for (let i = 0; i<sysData.DISK_info.length; i++) {
            diskData += `${sysData.DISK_info[i][0]} ${sysData.DISK_info[i][2]}GB`;
        }
        console.log(diskData);
        document.querySelector('.DISKused-container').innerHTML = diskData;
