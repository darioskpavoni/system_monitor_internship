...
setInterval(() => {
    socket.on('cpuUsage', (cpuUsage) => {
        ...
        if (document.getElementById(socket.id)) {
            let cpuUsageElement = document.getElementById(socket.id);
            cpuUsageElement.innerHTML = cpuUsage;
        }
        else {
            let cpuUsageElement = document.createElement('li');
            cpuUsageElement.style.display = 'inline';
            let cpuUsageSocket = socket.id;
            cpuUsageElement.innerHTML = `${cpuUsageSocket} has <p style="inline" id=${socket.id}>${cpuUsage}%</p>`;
            document.querySelector('ul').appendChild(cpuUsageElement);
        }
