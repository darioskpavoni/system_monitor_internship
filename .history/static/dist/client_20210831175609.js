import { io } from "static/node_modules/socket.io-client/dist/socket.io.min.js";
...
socket.on("sysData", (sysData) => {
  ...
  if (!document.getElementById(`${sysData.id}`)) {
    ...
    const htmlCPUchart = document.querySelector(
      `[id="${sysData.id}"] .CPUgraph`
    );
    ...
    const htmlRAMchart = document.querySelector(
      `[id="${sysData.id}"] .RAMgraph`
    );
    ...
    const htmlDISKchart = document.querySelector(
      `[id="${sysData.id}"] .DISKgraph`
    );
    ...
    // CPU CHART
    const htmlCPUchart = document.querySelector(
      `[id="${sysData.id}"] .graphsContainer .CPUgraph`
    );
    const CPUchart = echarts.init(htmlCPUchart);
    CPUchart.setOption(CPUchart_option);
    // RAM CHART
    const htmlRAMchart = document.querySelector(
      `[id="${sysData.id}"] .graphsContainer .RAMgraph`
    );
    const RAMchart = echarts.init(htmlRAMchart);
    RAMchart.setOption(RAMchart_option);
    // DISK CHART
    const htmlDISKchart = document.querySelector(
      `[id="${sysData.id}"] .graphsContainer .DISKgraph`
    );
    const DISKchart = echarts.init(htmlDISKchart);
    DISKchart_option.yAxis.data = [];
    for (let i = 0; i < sysData.DISK_used.length; i++) {
